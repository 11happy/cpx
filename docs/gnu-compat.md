...
Current state (Linux, non-root): 49 pass, 10 expected failures, 9 skipped.
...
Not covered by a test but worth knowing: a symlink given on the command line
of a non-recursive copy is copied as a symlink (cpx defaults to `-P`); GNU cp
follows it. Use `-L` or `-H` to follow.
