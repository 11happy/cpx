...
[ -x "$CPX" ] || { echo "SKIP: cpx not found"; exit 77; }

[ "$(id -u)" -eq 0 ] || { echo "SKIP: must run as root"; exit 77; }
command -v setcap >/dev/null 2>&1 || { echo "SKIP: setcap not found"; exit 77; }
command -v getcap >/dev/null 2>&1 || { echo "SKIP: getcap not found"; exit 77; }
...
setcap 'cap_net_bind_service=ep' file || { echo "SKIP: setcap doesn't work"; exit 77; }
getcap file | grep -q cap_net_bind_service || { echo "SKIP: getcap doesn't work"; exit 77; }
